...
def plot_connectivity(ax, PSET, cmap=plt.get_cmap('inferno'),
    ...
    masked_array = np.ma.array(PSET.connParams[data], mask=np.array(PSET.connParams[data])==0.)
    cmap.set_bad('k', 0.5)
...
if __name__ == '__main__':    
    ...
    fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_m_types.pdf'),
                bbox_inches='tight')
    plt.close(fig)
    ...
    fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_connectivity.pdf'),
                bbox_inches='tight')
    plt.close(fig)
    

    # plot layer specificity of connections between pre and postsynaptic cell types
    fig = plt.figure()
    ...
    fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_L_YXL.pdf'),
                        bbox_inches='tight')
    plt.close(fig)
    ...
    if RANK == 0 and PSET.COMPUTE_LFP:
        ...
        for data, title, suffix, color in zip(
                               [#f['SUMMED_OUTPUT'].value['imem'],
                                ...
                                # f['SUMMED_OUTPUT'].value['isyn_e'] + f['SUMMED_OUTPUT'].value['isyn_i'],                                
                                # f['SUMMED_OUTPUT'].value['imem'] - f['SUMMED_OUTPUT'].value['ipas'] - f['SUMMED_OUTPUT'].value['icap'] - f['SUMMED_OUTPUT'].value['isyn_e'] - f['SUMMED_OUTPUT'].value['isyn_i'],
                                ] + [f['SUMMED_OUTPUT'].value[name] for name in f['SUMMED_OUTPUT'].dtype.names],
            ...
            ax.set_title(title)
            vlimround = draw_lineplot(ax=ax, data=decimate(data, q=PSET.decimate_q),
            ...
            # save figure output
            fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_summed_{}.pdf'.format(suffix)),
                        bbox_inches='tight')
            plt.close(fig)
        f.close()
    ...
    if RANK == 0 and PSET.COMPUTE_LFP:
        ...
        for data, label, color in zip([#f['SUMMED_OUTPUT'].value['imem'],
                                      ...
                                      [#'sum',
                                       # r'$i_\mathrm{pas}$', r'$i_\mathrm{cap}$', r'$i_\mathrm{syn, E}$', r'$i_\mathrm{syn, I}$', r'$i_\mathrm{syn, E}+i_\mathrm{syn, I}$', 'residual'
                                       ] + [name for name in f['SUMMED_OUTPUT'].dtype.names],
                                      [#'k',
                                       # 'r', 'b', 'c', 'm', 'g', 'y'
                                       ] + ['k'] + [colors[i] for i in range(PSET.populationParameters.size)]):
            ax.semilogx(data[:, tind:].var(axis=1), y, lw=2, label=label, color=color)
        f.close()
        ax.set_yticks(y)
        ...
        ax.set_title(r'LP filtered signals ($f_\mathrm{crit}=100$ Hz, 4th order Butterworth, filtfilt)')
        ...
        for data, label, color in zip([#f['SUMMED_OUTPUT'].value['imem'],
            ...
            ax.semilogx(ss.filtfilt(b, a, data, axis=-1)[:, tind:].var(axis=1), y, lw=2, label=label, color=color)
        f.close()
        ax.set_yticks(y)
        ax.set_yticklabels(yticklabels)
        ax.axis(ax.axis('tight'))
        ...
        fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_variance.pdf'),
                    bbox_inches='tight')
        plt.close(fig)
    ...
    if RANK == 0:
        ...
        for i, name in enumerate(PSET.populationParameters['me_type']):
            ...
            for gid, spt in zip(f['SPIKES'][name]['gids'], f['SPIKES'][name]['times']):
                if len(spt) > 0:
                    ...
                    x += list(spt)
            ax.plot(x, y, '|',
                    color=colors[i], markersize=2, lw=2, clip_on=True, label=name)
        f.close()
        ax.axis(ax.axis('tight'))
        ...
        # save figure output
        fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_raster.pdf'),
                    bbox_inches='tight')
        plt.close(fig)
    ...
    if RANK == 0:
        ...
        axes[0].set_title(r'population spike time histogram, ($\Delta t={}$ ms)'.format(dt))
        
        for i, name in enumerate(PSET.populationParameters['me_type']):
            ax = axes[i]
            data = np.hstack(f['SPIKES'][name]['times'].value.flat)
            ax.hist(data, bins=bins, color=colors[i]) #, histtype='step', color=colors[i])
            ax.axis(ax.axis('tight'))
            ax.set_xlim(PSET.TRANSIENT, PSET.tstop)
        ...
        # save figure output
        fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_spike_time_histogram.pdf'),
                    bbox_inches='tight')
        plt.close(fig)
    ...
    if RANK == 0:
        ...
        fig.suptitle('per-cell spike count hist., T={} s'.format((PSET.tstop - PSET.TRANSIENT) / 1000.))
        
        bins = np.arange(42) * (PSET.tstop - PSET.TRANSIENT) / 1000. # make count bins conform to bin size of 1 Hz. 
        f = h5py.File(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_output.h5'), 'r')
        for i, name in enumerate(PSET.populationParameters['me_type']):
            ax = fig.add_subplot(gs[i // ncols, i % ncols])
            x = []
            for spt in f['SPIKES'][name]['times']:
                if spt.size == 0:
                    ...
                    if np.any(spt >= PSET.TRANSIENT):
                        ...
                        x += [0]
            ax.hist(x, bins=bins, color=colors[i], clip_on=True, label=name) # histtype='step', 
            ax.axis(ax.axis('tight'))
            ax.set_title(name)
        ...
        f.close()
        # save figure output
        fig.savefig(os.path.join(PSET.OUTPUTPATH, 'example_parallel_network_spike_count_hist.pdf'),
                    bbox_inches='tight')
        plt.close(fig)        